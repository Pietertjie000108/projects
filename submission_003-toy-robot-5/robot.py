...
def valid_command(command):
    ...
    if command_name.lower() == 'mazerun':
        if len(arg1.strip()) == 0:
            return True
        elif (arg1.lower().find('top') > -1 or arg1.lower().find('bottom') > -1 or \
             arg1.lower().find('left') > -1 or arg1.lower().find('right') > -1) \
...
def do_forward(robot_name, steps):
    ...
    """
    position = update_position(steps,robot_name)
    if  position == "obs":
...
def call_command(command_name, command_arg, robot_name):
    if command_name == 'help':
        ...
        return mazerun.mazerunner(robot_name, command_arg)
    return False, None
